...
last = 0
for key in results:
    if results[key]['time'] == None:
        continue
    if results[key]['time']>last:
        last = results[key]['time']
for key in results:
    if results[key]['time'] == None:
        continue
    progress = results[key]['time']/last
    myInternet.nodecolor[key] = (progress,progress,0.5+0.5*progress)
nx.draw_networkx(dg,arrows=False,with_labels=False,pos=pos,node_size=200,node_color=myInternet.nodecolor)
plt.show()
print len(myInternet.record)
