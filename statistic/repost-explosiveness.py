...
repost = []
exp = []
while explosiveness<1.01:
    exp.append(explosiveness)
    print explosiveness
    suma = 0
    for i in range(experiment):
        myCrowd = Crowd(0,0,0,100,netInitial=10)
        myInternet = internetMOD(myCrowd.dg,myCrowd.netInitial)
        myInternet.updateWithTime(totalTime)
        suma += myInternet.repostCount
    avg = suma * 1.0 / experiment
    repost.append(avg)
    explosiveness += timeInterval
print exp
print repost
